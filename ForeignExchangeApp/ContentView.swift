...
struct ContentView: View {
    ...
    @State private var someText = "Welcome to your currency converter"
 ...
 Text(someText)
    ...
    func fetchCurrency() {
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            if let decodedRates = try? JSONDecoder().decode(Rates.self, from: currencyData) {

                // DEBUG:
                print("Joke data decoded from JSON successfully")
                print("The rate is: \(decodedRates.base)")

                // Now, update the UI on the main thread
                DispatchQueue.main.async {

                    // Assign the result to the "someText" stored property
                    someText = decodedRates.base

                }

            } else {

                print("Could not decode JSON into an instance of the Currency rates structure.")
